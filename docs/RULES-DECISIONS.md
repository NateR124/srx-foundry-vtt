...
| R59 | Combat | Gas-vent mechanics: the catalog carries no effect columns for weapon mods, and the mod descriptions are empty. What does an attached Gas-vent do in this system's recoil model (a flat −1 hit for firing last phase, R3/R7)? | An attached Gas-vent (or Advanced Gas-vent) negates the recoil −1 on that weapon (`rules/combat.mjs` composeAttackModifiers.recoilComp) — recoil compensation is the mod's universal function across editions. Other mods (scopes, silencer, bipod) remain manual pending book text. | provisional |
